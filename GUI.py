...
class GUI:
    def __init__(self):
        ...
        self.create_button("Start", TILE_SIZE*3 + 40, TILE_SIZE*3, self.click)
        self.create_button("Create Target", 50, TILE_SIZE*3, self.click_create_target)
        self.create_button("QUIT", TILE_SIZE*2+ 20, TILE_SIZE * 9 + 25, self.on_close)

        #Hàm bên ngoài
        drawtable(self.canvasL, [[0] * N for _ in range(N)])
        drawtable(self.canvasR, [[0] * N for _ in range(N)])
...
#Xỷ lý sự kiện nut Relaod target
    def click_create_target(self):
        self.Target = target()
        drawtable(self.canvasR, self.Target)

#Xử lý sự kiện nhấn nút
    def click(self):
        if int(len(self.Target)) == 0:
            messagebox.askokcancel("Chưa có mục tiêu", "Mục tiêu chưa được chọn?")
        else:
            selected = self.combo.get()
            if selected == "BFS":
                ...
                initial = [[0] * N for _ in range(N)]
                answer = run_dfs(initial, self.Target)
                print(answer)
                if answer:
                    for state in answer:
                        drawtable(self.canvasL, state)
                ...
                initial = [[0] * N for _ in range(N)]
                answer = run_dls(initial, self.Target)
                if answer:
                    for state in answer:
                        drawtable(self.canvasL, state)
                        self.WINDOW.update()
                ...
                initial = [[0] * N for _ in range(N)]
                answer = run_ids(initial, self.Target)
                if answer:
                    for state in answer:
                        drawtable(self.canvasL, state)
                        self.WINDOW.update()
                ...
                if answer:
                    best = answer[len(answer) - 1]
                    temp = [[0] * N for _ in range(N)]
                    for r in range(N):
                        temp[r] = best[r][:]
                        drawtable(self.canvasL, temp)
                        self.WINDOW.update()
                        self.WINDOW.after(500)
                else:
                    print("Không tìm thấy nghiệm.")
            elif selected == "Genetic Algorithm":
                best, path = run_genetic_algorithm(self.Target)
                if best:
                        temp = [[0] * N for _ in range(N)]
                        for r in range(N):
                            temp[r] = best[r][:]
